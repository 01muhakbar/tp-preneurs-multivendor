...
- Initial Blueprint creation prompts for `DATABASE_URL`, `CLIENT_URL`, `CORS_ORIGIN`, and `PUBLIC_BASE_URL`.
- `JWT_SECRET` is generated by Render unless you set an existing secret manually.
- Set `CLIENT_URL`, `CORS_ORIGIN`, and `PUBLIC_BASE_URL` to the same HTTPS origin for the first single-origin deploy, for example `https://your-service.onrender.com`.
- `preDeployCommand` runs `pnpm -F server migrate`; keep `DB_SYNC=false` so production schema changes come only from migrations.
