...
// 6. renderStep(0)
const step0Regex = /if \(step === 0\) \{[\s\S]*?return \([\s\S]*?<>[\s\S]*?<div className="ssa-form-grid ssa-form-grid--two">[\s\S]*?<\/div>[\s\S]*?<div className="ssa-upload-grid">[\s\S]*?<\/div>[\s\S]*?<\/>[\s\S]*?\);[\s\S]*?\}/;
const newStep0 = `if (step === 0) {
      return (
        <>
...
content = content.replace(step0Regex, newStep0);

// 7. renderStep(2)
content = content.replace(
  /<section className="ssa-form-section">\s*<h3><MapPin size=\{18\} \/> 2\. Pickup \/ Warehouse Address<\/h3>\s*<div className="ssa-radio-row">[\s\S]*?<\/div>\s*\{\!form\.pickupSameAsBusiness && <Field[\s\S]*?\/>\}\s*<\/section>/,
  ""
);
