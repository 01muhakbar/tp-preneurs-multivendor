  ...
  try {
    ...
    await owner.page.route("**/sellerWorkspace2026Flags.js*", async (route) => {
      ...
      const mockedBody = body.replace(
        /export function isSeller2026CatalogProductionEnabled\s*\(\)\s*\{/,
        "export function isSeller2026CatalogProductionEnabled() { return false; "
      );
      await route.fulfill({ response, body: mockedBody });
    });

    await owner.page.goto(`/seller/stores/${fixture.storeSlug}/catalog/products`, {
    ...
    if (/Products \/ Product Catalog/i.test(normalizedText)) {
      throw new Error("Seller 2026 catalog rendered when flags were OFF.");
    }
    ...
    await owner.page.route("**/sellerWorkspace2026Flags.js*", async (route) => {
      ...
      const mockedBody = body.replace(
        /export function isSeller2026CatalogProductionEnabled\s*\(\)\s*\{/,
        "export function isSeller2026CatalogProductionEnabled() { return true; "
      );
      await route.fulfill({ response, body: mockedBody });
    });
    ...
    const isEnabled = await owner.page.evaluate(async () => {
      ...
      return mod.isSeller2026CatalogProductionEnabled();
    });
    console.log(`[DEBUG] isSeller2026CatalogProductionEnabled in browser = ${isEnabled}`);

    text = await owner.page.locator("body").innerText({ timeout: 10_000 });
    normalizedText = text.replace(/\s+/g, " ").trim();
    if (!/Products \/ Product Catalog/i.test(normalizedText)) {
      throw new Error(`Seller 2026 live catalog did not render with flags ON. Text: ${normalizedText.slice(0, 300)}`);
    }
    if (/MOCK_DATA|PREVIEW_MODE/i.test(normalizedText)) {
      throw new Error("Mock data marker found on live catalog.");
    }
    ...
    // Cross Store
    await owner.page.goto(`/seller/stores/${fixture.otherStoreSlug}/catalog/products`, {
      waitUntil: "networkidle",
      timeout: 45_000,
    });
    const crossText = await owner.page.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    if (!/Access Forbidden|permission/i.test(crossText) || /Other Demo Store.*Products/i.test(crossText)) {
      throw new Error("Cross-store catalog route did not render a safe forbidden state.");
    }
    results.crossStore = {
      status: "PASS",
    ...
    await member.page.goto(`/seller/stores/${fixture.storeSlug}/catalog/products`, {
      waitUntil: "networkidle",
      timeout: 45_000,
    });
    const memberText = await member.page.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    if (!/Products|Access Forbidden|permission/i.test(memberText)) {
      throw new Error("Role-limited member catalog route did not render read-safe or permission-safe UI.");
    }
    results.member = {
      status: "PASS",
    ...
    await publicPage.goto("/seller-2026/catalog/products", { waitUntil: "networkidle", timeout: 45_000 });
    const previewText = await publicPage.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    if (!/Products/i.test(previewText)) throw new Error("Preview catalog route did not render.");
