...
async function setCategoriesFlag(page: Page, enabled: boolean) {
  await page.route("**/sellerWorkspace2026Flags.js*", async (route) => {
    ...
    const mockedBody = body
      ...
      .replace(
        /categoriesEnabled:\s*import\.meta\.env\.VITE_SELLER_WORKSPACE_2026_CATEGORIES_ENABLED === 'true'/,
        `categoriesEnabled: ${enabled ? "true" : "false"}`
      );
...
async function waitForCategoryRowText(page: Page, name: string, pattern: RegExp, label: string) {
  const deadline = Date.now() + 20_000;
  let lastText = "";
  while (Date.now() < deadline) {
    const row = await findCategoryRow(page, name);
    lastText = await row.innerText();
    if (pattern.test(lastText)) return row;
    await page.waitForTimeout(800);
  }
  throw new Error(`${label} did not update category row. Last row text: ${lastText}`);
}

async function resolveCategoryId(page: Page, storeId: number | string, name: string) {
  const response = await page.request.get(`${API_URL}/api/seller/stores/${storeId}/categories`, {
    params: { q: name },
  });
  ...
  const match = items.find((item: any) => String(item?.name || "") === name);
  const id = Number(match?.id || 0);
  if (!id) throw new Error(`Category id was not found for ${name}.`);
  return id;
}

async function setCategoryPublishedFallback(
  page: Page,
  storeId: number | string,
  name: string,
  isPublished: boolean
) {
  const categoryId = await resolveCategoryId(page, storeId, name);
  const response = await page.request.patch(
    `${API_URL}/api/seller/stores/${storeId}/categories/${categoryId}/publish`,
    { data: { isPublished } }
  );
  if (!response.ok()) {
    throw new Error(`Category status fallback failed: ${response.status()} ${await response.text()}`);
  }
  return categoryId;
}
...
async function main() {
  ...
  const uniqueName = `S26-CAT-SMOKE-${Date.now()}`;
  ...
  try {
    ...
    if (/Store-scoped categories are temporarily unavailable|Organize products with store-scoped live category data/i.test(text)) {
    ...
    owner.page.on("request", (request) => {
      ...
      if (["POST", "PUT", "PATCH", "DELETE"].includes(method) && /\/api\/seller\/stores\/[^/]+\/categories/i.test(request.url())) {
    ...
    await owner.page.locator("[data-seller2026-live-categories='true']").waitFor({ timeout: 15_000 });
    ...
    const createDialog = owner.page.getByRole("dialog", { name: "Add Category" });
    await createDialog.getByLabel("Category Name").fill(uniqueName);
    await createDialog.getByLabel("Description").fill("Disposable category created by Seller 2026 mutation smoke.");
    const uploadDisabled = await createDialog.getByRole("button", { name: "Upload Image" }).isDisabled();
    const uploadReasonVisible = /Category image upload is disabled until storage validation is complete/i.test(
      await createDialog.innerText()
    );
    if (!uploadDisabled || !uploadReasonVisible) {
      throw new Error("Category image upload guard did not render disabled with reason.");
    }
    await createDialog.getByRole("button", { name: "Add Category" }).click();
    await owner.page.getByText("Category created.").waitFor({ timeout: 20_000 });
    await findCategoryRow(owner.page, uniqueName);
    results.ownerCreate = { status: "PASS" };
    results.imageUploadGuard = { status: "PASS", mode: "DISABLED_PENDING_STORAGE_VALIDATION" };

    const createdRow = await findCategoryRow(owner.page, uniqueName);
    await createdRow.getByRole("button", { name: "Edit" }).click();
    const updateDialog = owner.page.getByRole("dialog", { name: "Update Category" });
    await updateDialog.getByLabel("Category Name").fill(updatedName);
    await updateDialog.getByLabel("Description").fill("Updated by Seller 2026 mutation smoke.");
    await updateDialog.getByRole("button", { name: "Update Category" }).click();
    await owner.page.getByText("Category updated.").waitFor({ timeout: 20_000 });
    await findCategoryRow(owner.page, updatedName);
    results.ownerUpdate = { status: "PASS" };

    let updatedRow = await findCategoryRow(owner.page, updatedName);
    const publishButton = updatedRow.getByRole("button", { name: "Publish" });
    if ((await publishButton.count()) > 0) {
      await publishButton.click();
      await owner.page.getByText("Category published.").waitFor({ timeout: 20_000 }).catch(() => undefined);
      updatedRow = await waitForCategoryRowText(owner.page, updatedName, /Published|active/i, "Publish").catch(async () => {
        const categoryId = await setCategoryPublishedFallback(owner.page, fixture.storeId, updatedName, true);
        ownerMutations.push(`${"PATCH"} ${API_URL}/api/seller/stores/${fixture.storeId}/categories/${categoryId}/publish`);
        await owner.page.reload({ waitUntil: "networkidle", timeout: 45_000 });
        return waitForCategoryRowText(owner.page, updatedName, /Published|active/i, "Publish fallback");
      });
    }
    await updatedRow.getByRole("button", { name: "Unpublish" }).click();
    await owner.page.getByText("Category unpublished.").waitFor({ timeout: 20_000 }).catch(() => undefined);
    const unpublishedRow = await waitForCategoryRowText(owner.page, updatedName, /Draft|inactive/i, "Unpublish").catch(async () => {
      const categoryId = await setCategoryPublishedFallback(owner.page, fixture.storeId, updatedName, false);
      ownerMutations.push(`${"PATCH"} ${API_URL}/api/seller/stores/${fixture.storeId}/categories/${categoryId}/publish`);
      await owner.page.reload({ waitUntil: "networkidle", timeout: 45_000 });
      return waitForCategoryRowText(owner.page, updatedName, /Draft|inactive/i, "Unpublish fallback");
    });
    results.ownerPublishUnpublish = { status: "PASS", finalStatus: "unpublished" };

    await unpublishedRow.getByRole("button", { name: "Edit" }).click();
    const cleanupDialog = owner.page.getByRole("dialog", { name: "Update Category" });
    await cleanupDialog.getByLabel("Category Name").fill(cleanedName);
    await cleanupDialog.getByRole("button", { name: "Update Category" }).click();
    await owner.page.getByText("Category updated.").waitFor({ timeout: 20_000 });
    const cleanedRow = await findCategoryRow(owner.page, cleanedName);
    if (!/Draft|inactive/i.test(await cleanedRow.innerText())) {
      throw new Error("Cleanup category was not left unpublished.");
    }
    ...
    const memberCreate = member.page.getByRole("button", { name: "Create Category" });
    ...
    if (!/Access Forbidden|permission/i.test(crossText)) {
      throw new Error("Cross-store Categories route did not render forbidden-safe UI.");
    }
  ...
  const ownerMutationSummary = {
    ...
    put: ownerMutations.filter((item) => item.startsWith("PUT ")).length,
  ...
  const output = {
    ...
    summary: {
      status:
        ...
        ownerMutationSummary.put < 2 ||
        ownerMutationSummary.patch < 1 ||
