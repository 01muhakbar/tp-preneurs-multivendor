...
async function setPaymentReviewFlag(page: Page, enabled: boolean) {
  await page.route("**/sellerWorkspace2026Flags.js*", async (route) => {
    ...
    const mockedBody = body
      ...
      .replace(
        /paymentCenterEnabled:\s*import\.meta\.env\.VITE_SELLER_WORKSPACE_2026_PAYMENT_CENTER_ENABLED === 'true'/,
        `paymentCenterEnabled: ${enabled ? "true" : "false"}`
      )
      .replace(
        /paymentReviewEnabled:\s*import\.meta\.env\.VITE_SELLER_WORKSPACE_2026_PAYMENT_REVIEW_ENABLED === 'true' \|\|\s*import\.meta\.env\.VITE_SELLER_WORKSPACE_2026_PAYMENT_CENTER_ENABLED === 'true'/,
        `paymentReviewEnabled: ${enabled ? "true" : "false"}`
      );
...
async function main() {
  ...
  try {
    ...
    await setPaymentReviewFlag(owner.page, false);
    await owner.page.goto(`/seller/stores/${fixture.storeSlug}/payment-review`, {
      waitUntil: "networkidle",
      timeout: 45_000,
    });
    if ((await owner.page.locator("[data-seller2026-live-payment-review='true']").count()) > 0) {
      throw new Error("Seller 2026 Payment Review rendered when flags were OFF.");
    }
    ...
    if ((await owner.page.locator("[data-seller2026-live-order-detail='true']").count()) > 0) {
      throw new Error("Seller 2026 Payment Review detail rendered when flags were OFF.");
    }
    ...
    owner.page.on("request", (request) => {
      ...
      if (["POST", "PUT", "PATCH", "DELETE"].includes(method) && /\/api\/seller\/stores\/[^/]+\/payments\/[^/]+\/review/i.test(url)) {
        ownerMutations.push({ method, url, payload: parseJsonPostData(request.postData()) });
      }
    });
    await setPaymentReviewFlag(owner.page, true);
    await owner.page.goto(`/seller/stores/${fixture.storeSlug}/payment-review`, {
      waitUntil: "networkidle",
      timeout: 45_000,
    });
    await owner.page.locator("[data-seller2026-live-payment-review='true']").waitFor({ timeout: 20_000 });
    const listText = await owner.page.locator("body").innerText({ timeout: 10_000 });
    if (!/Payment Review|Total orders|Payment|Fulfillment/i.test(listText)) {
      throw new Error("Live Payment Review list did not render expected production content.");
    }
    if (/Delete selected|Bulk|Update payment|Refund|Dispute/i.test(listText)) {
      throw new Error("Unsafe order action appeared on Seller 2026 Payment Review list.");
    }
    ...
    if (!/Approve payment|Reject payment/i.test(detailText)) {
      throw new Error("Payment Review detail did not render Approve or Reject actions.");
    }
    

    const approveBtn = owner.page.getByRole("button", { name: /Approve Payment/i }).first();
    if ((await approveBtn.count()) > 0 && !(await approveBtn.isDisabled())) {
      await approveBtn.click();
      await owner.page.getByText(/Payment approved/i).first().waitFor({ timeout: 20_000 });
      await owner.page.waitForLoadState("networkidle", { timeout: 45_000 });
      results.paymentAction = { status: "PASS", action: "APPROVE" };
    } else {
      results.paymentAction = { status: "DISABLED_SAFE", reason: "Approve was not exposed by backend governance." };
    }
    ...
    const paymentMutations = ownerMutations.filter((entry) => /payment/i.test(entry.url) && !/\/payments\/[^/]+\/review/i.test(entry.url));
    ...
    await setPaymentReviewFlag(member.page, true);
    member.page.on("request", (request) => {
      const method = request.method().toUpperCase();
      if (["POST", "PUT", "PATCH", "DELETE"].includes(method) && /\/api\/seller\/stores\/[^/]+\/payments\/[^/]+\/review/i.test(request.url())) {
        blockedMutations.push(`member ${method} ${request.url()}`);
      }
    });
    await member.page.goto(`/seller/stores/${fixture.storeSlug}/payment-review`, {
      waitUntil: "networkidle",
      timeout: 45_000,
    });
    await member.page.locator("[data-seller2026-live-payment-review='true']").waitFor({ timeout: 20_000 });
    ...
    await owner.page.goto(`/seller/stores/${fixture.otherStoreSlug}/payment-review`, {
      waitUntil: "networkidle",
      timeout: 45_000,
    });
    const crossText = await owner.page.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    if (!/Access Forbidden|permission/i.test(crossText)) throw new Error("Cross-store Payment Review list did not render forbidden-safe UI.");
    
    const crossDetailText = await owner.page.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    if (!/Access Forbidden|permission/i.test(crossDetailText)) throw new Error("Cross-store Payment Review detail did not render forbidden-safe UI.");
    ...
    await previewPage.goto("/seller-2026/payment-review", { waitUntil: "networkidle", timeout: 45_000 });
    const previewText = await previewPage.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    if (!/Payment Review|Fulfillment/i.test(previewText)) throw new Error("Preview Payment Review route did not render.");
  ...
  const output = {
    ...
    summary: {
      status:
        ...
        ownerMutations.some((entry) => entry.method === "DELETE" || /bulk-delete/i.test(entry.url) || (/payment/i.test(entry.url) && !/\/payments\/[^/]+\/review/i.test(entry.url))) ||
