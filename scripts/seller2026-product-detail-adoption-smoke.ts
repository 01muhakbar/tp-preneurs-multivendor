...
async function main() {
  ...
  if (!product) throw new Error("No product found in fixture store to test detail route.");
  ...
  try {
    ...
    await owner.page.route("**/sellerWorkspace2026Flags.js*", async (route) => {
      ...
      const mockedBody = body.replace(
        /export function isSeller2026ProductDetailProductionEnabled\s*\(\)\s*\{/,
        "export function isSeller2026ProductDetailProductionEnabled() { return false; "
      );
    ...
    if (/Product Detail \/ Preview/i.test(normalizedText)) {
      throw new Error("Seller 2026 product detail rendered when flags were OFF.");
    }
    ...
    await owner.page.route("**/sellerWorkspace2026Flags.js*", async (route) => {
      ...
      const mockedBody = body.replace(
        /export function isSeller2026ProductDetailProductionEnabled\s*\(\)\s*\{/,
        "export function isSeller2026ProductDetailProductionEnabled() { return true; "
      );
      await route.fulfill({ response, body: mockedBody });
    });
    ...
    // Debug what the function evaluates to in the browser context
    const isEnabled = await owner.page.evaluate(async () => {
      // Use dynamic import to get the currently loaded module
      const mod = await import('/src/features/sellerWorkspace2026/sellerWorkspace2026Flags.js');
      return mod.isSeller2026ProductDetailProductionEnabled();
    });
    console.log(`[DEBUG] isSeller2026ProductDetailProductionEnabled in browser = ${isEnabled}`);

    text = await owner.page.locator("body").innerText({ timeout: 10_000 });
    normalizedText = text.replace(/\s+/g, " ").trim();
    if (!/Product detail|Product Detail \/ Preview/i.test(normalizedText)) {
      throw new Error(`Seller 2026 live product detail did not render with flags ON. Text: ${normalizedText.slice(0, 300)}`);
    }
    if (/MOCK_DATA|PREVIEW_MODE/i.test(normalizedText)) {
      throw new Error("Mock data marker found on live product detail.");
    }
    results.flagsOn = { status: "PASS", snippet: normalizedText.slice(0, 180) };

    // Legacy Redirects
    await owner.page.goto(`/seller/stores/${fixture.storeSlug}/catalog`, { waitUntil: "networkidle", timeout: 45_000 });
    if (!owner.page.url().includes("/catalog/products")) {
      throw new Error("Legacy catalog route did not redirect to canonical products route.");
    }
    results.legacyRedirect = { status: "PASS", finalUrl: owner.page.url().replace(CLIENT_URL, "") };

    // Cross Store
    await owner.page.goto(`/seller/stores/${fixture.otherStoreSlug}/catalog/products/${productId}`, {
      waitUntil: "networkidle",
      timeout: 45_000,
    });
    const crossText = await owner.page.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    if (!/Access Forbidden|permission/i.test(crossText) || /Other Demo Store.*Product detail/i.test(crossText)) {
      throw new Error("Cross-store product detail route did not render a safe forbidden state.");
    }
    results.crossStore = {
      status: "PASS",
    ...
    await member.page.goto(`/seller/stores/${fixture.storeSlug}/catalog/products/${productId}`, {
      waitUntil: "networkidle",
      timeout: 45_000,
    });
    const memberText = await member.page.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    if (!/Product detail|Access Forbidden|permission/i.test(memberText)) {
      throw new Error("Role-limited member product detail route did not render read-safe or permission-safe UI.");
    }
    results.member = {
      status: "PASS",
    ...
    await publicPage.goto(`/seller-2026/catalog/products/${productId}`, { waitUntil: "networkidle", timeout: 45_000 });
    const previewText = await publicPage.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    if (!/Product detail/i.test(previewText)) throw new Error("Preview product detail route did not render.");
    ...
    const dashboardText = await publicPage.locator("body").innerText({ timeout: 10_000 }).catch(() => "");
    results.dashboardRegression = { status: "PASS", snippet: dashboardText.replace(/\s+/g, " ").trim().slice(0, 180) };
