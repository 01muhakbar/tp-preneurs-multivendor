...
export function createSeller2026StoreProfileForm(profile: any) {
  ...
  return {
    ...
    district: asText(profile?.district),
    province: asText(profile?.province),
    postalCode: asText(profile?.postalCode),
    country: asText(profile?.country, "Indonesia"),
    
    // Store Identity
    category: asText(profile?.businessDetails?.category),
    
    // Owner Information
    ownerName: asText(profile?.ownerIdentity?.fullName),
    ownerEmail: asText(profile?.ownerIdentity?.email || profile?.email),
    ownerPhone: asText(profile?.ownerIdentity?.phoneNumber || profile?.phone),
    birthDate: asText(profile?.ownerIdentity?.birthDate),
    identityType: asText(profile?.ownerIdentity?.identityType),
    identityNumber: asText(profile?.ownerIdentity?.identityNumber),
    taxNumber: asText(profile?.ownerIdentity?.taxNumber),
    residentialAddress: asText(profile?.ownerIdentity?.residentialAddress),
    ownerCountry: asText(profile?.ownerIdentity?.country, "Indonesia"),
    ownerProvince: asText(profile?.ownerIdentity?.province),
    ownerCity: asText(profile?.ownerIdentity?.city),
    ownerSubdistrict: asText(profile?.ownerIdentity?.subdistrict),
    ownerPostalCode: asText(profile?.ownerIdentity?.postalCode),

    // Business Details
    businessType: asText(profile?.businessDetails?.businessType),
    legalEntity: asText(profile?.businessDetails?.legalEntity),
    businessCountry: asText(profile?.country, "Indonesia"),
    businessProvince: asText(profile?.province),
    businessCity: asText(profile?.city),
    businessPostalCode: asText(profile?.postalCode),
    businessAddress: asText(profile?.addressLine1),
    timeZone: asText(profile?.businessDetails?.timeZone),
    openTime: asText(profile?.businessDetails?.openTime),
    closeTime: asText(profile?.businessDetails?.closeTime),
    workingDays: asText(profile?.businessDetails?.workingDays),
    shippingMethod: asText(profile?.businessDetails?.shippingMethod),
    processingTime: asText(profile?.businessDetails?.processingTime),
