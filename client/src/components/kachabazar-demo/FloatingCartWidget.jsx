...
export default function FloatingCartWidget() {
  ...
  return (
    ...
    >
      <span className="relative mt-4 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#f7fbff] text-[var(--tp-primary)] shadow-inner dark:bg-slate-800 dark:text-sky-300">
        <ShoppingCart className="h-6 w-6" />
        {count > 0 ? (
          <span className="absolute -right-1 -top-1 inline-flex min-w-5 items-center justify-center rounded-full bg-[var(--tp-accent)] px-1.5 text-[11px] font-black leading-5 text-white">
            {count}
          </span>
        ) : null}
      </span>
      ...
      <span className="mt-4 block w-full bg-[var(--tp-primary)] px-2 py-4 text-center text-lg font-black leading-none text-white">
